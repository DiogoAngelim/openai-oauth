...
describe('OpenAIController', () => {
  ...
  it('should handle chat with stream=false', async () => {
    const req = { user: { orgId: 'org1', sub: 'user1' }, get: jest.fn(), header: jest.fn(), accepts: jest.fn() } as any
    const res = { json: jest.fn(), status: jest.fn(), sendStatus: jest.fn(), links: jest.fn(), send: jest.fn() } as any
  ...
  it('should handle chat with stream=true', async () => {
    const req = { user: { orgId: 'org1', sub: 'user1' }, get: jest.fn(), header: jest.fn(), accepts: jest.fn() } as any
    const res = { setHeader: jest.fn(), write: jest.fn(), end: jest.fn(), status: jest.fn(), sendStatus: jest.fn(), links: jest.fn(), send: jest.fn() } as any
    const body = { prompt: 'Hello' }
