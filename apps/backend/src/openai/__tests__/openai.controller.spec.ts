...
describe('OpenAIController', () => {
  ...
  it('should handle chat with stream=false', async () => {
    const req = { user: { orgId: 'org1', sub: 'user1' }, get: jest.fn(), header: jest.fn(), accepts: jest.fn() } as unknown as import('express').Request & { user: { orgId: string; sub: string } };
    const res = { json: jest.fn(), status: jest.fn(), sendStatus: jest.fn(), links: jest.fn(), send: jest.fn() } as unknown as import('express').Response;
  ...
  it('should handle chat with stream=true', async () => {
    const req = { user: { orgId: 'org1', sub: 'user1' }, get: jest.fn(), header: jest.fn(), accepts: jest.fn() } as unknown as import('express').Request & { user: { orgId: string; sub: string } };
    const res = { setHeader: jest.fn(), write: jest.fn(), end: jest.fn(), status: jest.fn(), sendStatus: jest.fn(), links: jest.fn(), send: jest.fn() } as unknown as import('express').Response;
    const body = { prompt: 'Hello' };
    openai.createChatCompletion.mockImplementation(async (_orgId, _userId, _body, _stream, cb) => {
      cb('chunk1');
      cb('chunk2');
    });
